      ...
      begin
        ...
        | Type.Timestamp ->
          ...
          if not (HighTerm.is_constant `Exact env f &&
            ...
            hard_failure ~loc
              (Failure "global case must be on a deterministic and \
...
let old_induction Args.(Message (ts,_)) s =
  ...
  if not (HighTerm.is_constant `Exact env ts &&
    ...
    hard_failure 
      (Failure "simple global induction must be on a deterministic and \
  ...
  let subgoal_of_occ (vs,is,t) =
    ...
    | Term.Diff (Explicit [p1,s1; p2,s2]) as subterm
      when p1 = l_proj && p2 = r_proj ->
        ...
        Goal.Equiv
          (ES.set_goal
             (* TODO: we assume that the variables are global and deterministic. 
    ...
    List.map (fun (p_bool, p_term) ->
        ...
        if not (HighTerm.is_constant `Exact (ES.env s) t_bool &&
          ...
          hard_failure ~loc:p_bool_loc
            (Failure "conditions must be deterministic and system-independent");
