...
type allowed_constants = Det | PTimeSI | PTimeNoSI
...
let get_macro_occs
  ...
  let rec get (t : Term.term) ~(fv:Vars.vars) ~(cond:Term.terms) : macro_occs =
    ...
    | _ when mode = Det       && HighTerm.is_constant        `Exact           env t -> []

    | Term.Var v -> 
      let err_str =
        Fmt.str "terms contain a %s variable: @[%a@]" 
          (match mode with Det -> "non-deterministic" | PTimeSI | PTimeNoSI -> "non-ptime")
          Vars.pp v
      in
