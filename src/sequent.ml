...
= struct
  ...
  let error_pt_apply_not_system_indep loc ~(pt : PT.t) ~(arg : Term.t) =
    ...
    (*------------------------------------------------------------------*)
  let error_pt_apply_not_deterministic loc ~(pt : PT.t) ~(arg : Term.t) =
    let err_str =
      Fmt.str "@[<v 0>The term:@;  @[%a@]@;\
               is not deterministic. It cannot be applied to:@;  @[%a@].@]"
        Term.pp arg
        PT.pp pt
    in
    soft_failure ~loc (Failure err_str)
  ...
  let pt_apply_var_forall
    ...
    let () =
      ...
      if f_arg_tag.const && not (HTerm.is_constant `Exact env pt_arg) then
        error_pt_apply_not_deterministic arg_loc ~pt ~arg:pt_arg;
    in
