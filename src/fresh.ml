...
let get_bad_occs
  ...
  | Var v ->
    soft_failure
      (Failure (Fmt.str "terms contain a non-deterministic variable: %a" Vars.pp v))
...
let fresh_trace
  ...
  try
    ...
    let phis =
      NO.name_occurrence_formulas ~mode:Iter.Det ~use_path_cond ~pp_ns:(Some pp_n)
...
let equiv_fresh_phi_proj
  ...
  let phi : Term.terms =
    NO.name_occurrence_formulas
      ~mode:Iter.Det ~use_path_cond ~negate:true ~pp_ns:(Some pp_n)
