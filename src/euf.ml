...
let get_bad_occs
  ...
  let rec_call ?(fv = fv) = (* rec call on a list *)
  ...
  | Fun (f, _, [Tuple [m'; tk']]) when f = int_f ->
    begin
      ...
      | Name (ksb', kargs') as k' when k.symb.s_symb = ksb'.s_symb  ->
        let fvv, sigma = Term.refresh_vars fv in
        let kargs' = List.map (Term.subst sigma) kargs' in
        ...
        let occs, accs = rec_call ~fv:fvv (m' :: kargs') in
        occs,
